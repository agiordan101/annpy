...
# 		# if len(axes) != 3:
# 		# 	raise Exception("[annpy error] add_graph: Parameter 'axes' len need to be 3")
...
# 			raise Exception(f"[annpy error] add_curve: Unable to find {graph} graph")

# 		if self.graphs[graph]['x']:
# 			if range(len(curve)) != self.graphs[graph]['x']:
# 				raise Exception(f"[annpy error] add_curve: x axis of graph {graph} are not the same: {range(len(curve))} != {self.graphs[graph]['x']}")
# 		else:
